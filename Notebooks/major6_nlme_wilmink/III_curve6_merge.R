...
# Loading data: 
load("M:/PCR_data/curve6_prepare.RData")
rm(major6, minor, dryoff); gc()
...
df_model <- df_model %>%
  # we have already filtered in PCR_clean: SCC > 0, before log transforming
  filter(logSCC > 0) %>%
  filter(PARITY > 1)
...
save.image("M:/PCR_data/curve6_merge.RData") 
