...
mink_neg4 <- out_neg4 %>% slice_max(out_neg4$k, prop = 0.9) # k
mind_neg4 <- out_neg4 %>% slice_max(out_neg4$d, prop = 0.09) # d
...
mink_neg4[,"BES_ID"] # "3267112" "4278212"
mink_neg4 <- dplyr::filter(df4_neg, grepl('3267112|4278212', BES_ID))
mind_neg4[,"BES_ID"] # "2587212" "4650012"
mind_neg4 <- dplyr::filter(df4_neg, grepl('2587212|4650012', BES_ID))
...
ggplot(df2_pos, aes(logSCC)) + 
  stat_ecdf(geom = "step") + 
  xlim(0.0, 9.0) +
  theme(legend.position="none") + 
  theme_bw() + 
  labs(title="parity 2 - pos")

ggplot(df2_neg, aes(logSCC)) + 
  stat_ecdf(geom = "step") + 
  xlim(0.0, 9.0) +
  theme(legend.position="none") + 
  theme_bw() + 
  labs(title="parity 2 - neg")
