...
# I might need a 1 on left side (before log transforming)
...
# Model: logtSCC ~ a + b * DIM + exp(-(exp(k)) * DIM)*d, data = df_boo2
...
nls_pos <- nls(formula = logtSCC ~ a + b * DIM + exp(-(exp(k)) * DIM)*d, data = df2_pos, 
             start = list(a = 7.9, b = 0.0018, k = -1.6, d = 2.0),
...
x_eval <- seq(min(df2_pos$DIM), max(df2_pos$DIM), length.out = 100) # X axis (DIM alredy set to 0-305 days)
...
# NEG
nls_neg <- nls(formula = logtSCC ~ a + b * DIM + exp(-(exp(k)) * DIM)*d, data = df2_neg, 
               start = list(a = 7.9, b = 0.0018, k = -1.6, d = 2.0),
               control = list(maxiter = 100, tol = 1e-05, minFactor = 1/1024, 
                              printEval = FALSE, warnOnly = TRUE), 
               algorithm = "port", trace = FALSE)
...
x_eval <- seq(min(df2_neg$DIM), max(df2_neg$DIM), length.out = 100) # X axis (DIM alredy set to 0-305 days)
...
p_pos <- ggplot(data = Estims_pos, aes(x = x, y = median_est, ymin = ci_lower_est, ymax = ci_upper_est)) + 
  ...
  ylim(7.5, 8.7) +
  #geom_point(data = df_boo2, aes(x = DIM, y = logtSCC), size = rel(1), colour = "darkgreen", inherit.aes = FALSE) + 
  theme_bw() + labs(title = "Wilmink for: PCR POS, parity 2, Holstein", x = "DIM", y = "logtSCC")
...
p_neg <- ggplot(data = Estims_neg, aes(x = x, y = median_est, ymin = ci_lower_est, ymax = ci_upper_est)) + 
  ...
  ylim(7.5, 8.7) +
  #geom_point(data = df_boo2, aes(x = DIM, y = logtSCC), size = rel(1), colour = "darkgreen", inherit.aes = FALSE) + 
  theme_bw() + labs(title = "Wilmink for: PCR NEG, parity 2, Holstein", x = "DIM", y = "logtSCC")
#ggsave("bootpstrap_results.pdf", height = 5, width = 9)
p_neg
...
p2_SCC <- ggplot() + 
  ...
  ylim(7.5, 8.7) +
  theme_bw() + labs(title = "Parity 2: Red = NEG, Green = POS", x = "DIM", y = "logtSCC")
#ggsave("bootpstrap_results.pdf", height = 5, width = 9)
...
p3_SCC <- ggplot() + 
  ...
  geom_line(data = Estims_pos, aes(x = x, y = median_est), size = rel(1.5), colour = "darkgreen") + 
  ylim(3.5, 6.0) +
  theme_bw() + labs(title = "Parity 3: Red = NEG, Green = POS", x = "DIM", y = "logtSCC")
#ggsave("bootpstrap_results.pdf", height = 5, width = 9)
