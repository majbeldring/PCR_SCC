...
df2 <- df2 %>% filter(BES_ID == 5051112)
df2 <- df2 %>% 
  ungroup()%>%
  dplyr::select(-BES_ID)
df2$DYR_ID <- factor(df2$DYR_ID) # removed unused levels by resetting
df2$RES_MAJOR <- factor(df2$RES_MAJOR) 
...
# herd 5051112
dplyr::n_distinct(df2$DYR_ID) # 256 (2249 obs)
dplyr::n_distinct(df2_pos$DYR_ID) # 85 (740 obs)
dplyr::n_distinct(df2_neg$DYR_ID) # 171 (1509) 
...
df2 %>%
  ...
  ggtitle("herd 5051112: logSCC distibution parity 2") +
...
wilmink_nlme_fit <- nlme(logSCC ~ a + b * DIM/150 + exp(-(exp(k)) * DIM/150)*d, 
                         ...
                         groups = ~ DYR_ID, 
                         #start = wilmink_nls_out_summary$median,
                         start = c(a=4, b=0.0039, k=-2.1, d=1.8),
                         na.action=na.exclude,
