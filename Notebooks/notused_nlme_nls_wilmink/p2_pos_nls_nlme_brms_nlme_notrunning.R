...
df2$BES_ID <- factor(df2$BES_ID) 
df2$DYR_ID <- factor(df2$DYR_ID) 
...
# herd 5051112
dplyr::n_distinct(df2$DYR_ID) # 45731 (393.826 obs)
...
plot_nls <- function(nls_object, data) {
  ...
  ggplot(data, aes(x=DIM, y=logSCC)) +
    ...
    ggtitle("parity 2 POS: nls fit") +
    geom_line(data = predframe, aes(x=DIM, y=logSCC_nls))
}

plot_nls(wilmink_nls, df2)
...
# a           b           k           d 
# 4.03167581  0.00387419 -2.11002380  1.83918835  

coef(wilmink_nls2) # not multistart
# a            b            k            d 
# 4.031686536  0.003874139 -2.109845980  1.839521108 
...
df_nls <- df2 %>%
  nest(df2_data = c(BES_ID, DIM, logSCC)) %>% 
  mutate(wilmink_nls = map(df2_data, ~wilmink_nls_fit_func(.x)))
...
hght.quad.nlme <- nlme(logSCC ~ a + b * DIM + exp(-(exp(k)) * DIM)*d,
                       data=df2,
                       fixed= a + b + k + d ~ 1, 
                       random = a + b + k + d ~ 1,
                       groups=~BES_ID,
                       start=c(4.032, 0.00387, -2.1098, 1.8395),
                       na.action=na.exclude,
                       control=(msMaxIter = 10000))
...
test_nlme <- nlme(logSCC ~ a + b * DIM + exp(-(exp(k)) * DIM)*d,
                  ...
                  random=a+b+k+d~1,
                  groups=~BES_ID,
                  start=c(a = 4.032, b = 0.00387, k = -2.1098, d = 1.8395),
                  na.action=na.exclude,
                  control=(msMaxIter = 2000))
