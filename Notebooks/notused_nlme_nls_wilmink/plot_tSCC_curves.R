...
# define wilmink eq 8:
logtSCC_func <- function(DIM, a,b,k,d) 
...
pos2_pct %>% 
  ...
  #' calculate the proper `logtSCC`
  mutate(logtSCC = pmap_dbl(select(., DIM, a,b,k,d), logtSCC_func)) %>% {
    ggplot(., aes(DIM, logtSCC)) + 
      # aes(group = BES_ID) +
      aes(group = pctile) +
      aes(color = pctile) +
      geom_line() +
...
# plot all tSCC medians:
grid.arrange(p2_pos_median, p2_neg_median, ncol=2, top="Parity 2 - tSCC")
