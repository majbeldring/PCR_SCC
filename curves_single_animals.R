...
# dividing into parity 2 - both POS and NEG
df2 <- df %>%
  filter(PARITY == 2) %>%
  ungroup() %>%
  group_by(DYR_ID) %>%
...
df2$DYR_ID <- factor(df2$DYR_ID) # keep only used levels by resetting the variable

# dividing into POS and NEG
df2_pos <- df %>%
  filter(RES_MAJOR == 1) %>%
  ungroup() %>%
...
df2_pos$DYR_ID <- factor(df2_pos$DYR_ID)
df2_neg$DYR_ID <- factor(df2_neg$DYR_ID)
...
ggplot(df2_pos, aes(x=DIM, y=logSCC)) +
  geom_point(size=1) +
  ylim(c(0, 10))
